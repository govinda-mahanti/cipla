...
const DoctorsList = () => {
    ...
    const fetchDoctors = async () => {
      try {
        const response = await axios.get(
          `https://cipla-backend.virtualspheretechnologies.in/api/getTotalDoctorsByManager?emp_code=${id}`,
          {
            headers: {
  ...
  const handleSearch = () => {
    const filtered = doctorData.filter((doc) => {
      const nameMatch = doc.doctor_full_name
        ?.toLowerCase()
          ...
          {currentData.length > 0 ? (
            currentData.map((doc, index) => (
              <tr key={index} className="text-sm hover:bg-gray-50">
                ...
                <td className="py-2 px-4 border">{doc.doctor_full_name}</td>
                      ...
                      <button
                        onClick={() =>
                          handleDownload(
                            ...
                            `doctor_${doc.doctor_full_name || "image"}.jpg`
                          )
                        }
                      ...
                      <button
                        onClick={() =>
                          handleDownload(
                            ...
                            `doctor_${doc.doctor_full_name || "video"}.mp4`
                          )
                        }
            ...
            <tr>
              <td colSpan="6" className="py-3 px-4 border text-center text-gray-500">
                No doctors found.
              </td>
            </tr>
          )}
      ...
      {showModal && selectedDoctor && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded shadow-md w-[90%] max-w-[500px]">
            <h3 className="text-lg font-bold mb-4 text-center">Media Preview</h3>
